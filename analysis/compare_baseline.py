...
# 文件名列表，假设你的文件放在当前目录下
files = ['./result/epom/epom-best.jsonl',
         './result/baseline/epom-对比-在理想视野下训练的带旋转动作-在vpo-mapf环境下测试.jsonl',]  # 添加你的文件名
algorithm_names = ['V-EPOM' ,'EPOM']
...
def plot_metric(metric, ylabel, title):
    ...
    plt.legend()
    plt.grid(True)
    # plt.savefig(f'Baseline-{ylabel}.png')
    plt.show()
